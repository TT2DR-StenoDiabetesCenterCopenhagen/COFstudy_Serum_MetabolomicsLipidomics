...
met_data <- combined_data %>% 
  select(all_of(metabol_names)) %>% 
  t()
colnames(met_data) <- combined_data$id_bwcat_NAFLD #colnames(metabo_data)[4:ncol(metabo_data)]
rownames(met_data) <- metabol_names

# Prepare data format of clinical data 
clin_data <- combined_data %>%  
  filter(Label == "A") %>% 
  select(everything(), -all_of(colnames(metabo_data))) %>% 
...
pca_object <- pca(as.data.frame(met_data), metadata = as.data.frame(clin_data), removeVar = 0.1)
```
...
biplot_loadings <- biplot(pca_object, showLoadings = TRUE,
    ...
    # ellipse config
      ...
      ellipseLineSize = 1.0,
    legendPosition = 'right',
    xlim = c(-15,15), ylim = c(-10, 15))
biplot_loadings
ggsave(paste(path, "results/03_02_biplot_loadings.png", sep="/"), plot = biplot_loadings, device = "png", width = 10, height = 10)
```

**Bi-plot:** A plot that plots both variables and observations (samples) in the same space. The variables are indicated by arrows drawn from the origin, which indicate their ‘weight’ in different directions. One outlier is detected (NBW_FALSE)
...
combined_data_baseline_outlier <- combined_data %>% 
  filter(Label == "A") %>%
  unite("id_bwcat_NAFLD", c(Sample_ID, bwcat_NAFLD), remove = FALSE) %>% 
  filter(id_bwcat_NAFLD != "066_LBW_FALSE") #, id_bwcat_NAFLD != "033_LBW_TRUE") # One outlier has been removed

# Prepare data format of metabolomic data 
metabol_names <- colnames(metabo_data)[3:ncol(metabo_data)]
met_data <- combined_data_baseline_outlier %>% 
  select(all_of(metabol_names)) %>% 
  t()
colnames(met_data) <- combined_data_baseline_outlier$id_bwcat_NAFLD #colnames(metabo_data)[4:ncol(metabo_data)]
rownames(met_data) <- metabol_names
...
pca_object$metadata$bwcat_NAFLD <- factor(pca_object$metadata$bwcat_NAFLD , levels=c("NBW_FALSE", "LBW_FALSE", "LBW_TRUE"))

biplot_loadings <- biplot(pca_object, showLoadings = TRUE,
                          colkey = c('NBW_FALSE' = '#32CD32', 'LBW_FALSE' = '#6495ED', 'LBW_TRUE' = '#FA8072'), 
    labSize = 3, pointSize = 3, sizeLoadingsNames = 3, lab = NULL, colby = 'bwcat_NAFLD', 
...
biplot_loadings
ggsave(paste(path, "results/03_04_biplot_loadings_outlierremoved.png", sep="/"), plot = biplot_loadings, device = "png", width = 13, height = 10)
```
...
loadingsplot <- plotloadings(pca_object, components = getComponents(pca_object, 1:horn$n), 
                             ...
                             drawConnectors = TRUE)
loadingsplot
ggsave(paste(path, "results/03_07_loadings.png", sep="/"), plot = loadingsplot, device = "png", width = 15, height = 15)
```
...
```{r echo=FALSE}
PC6_plotloadings <- plotloadings(pca_object,
  components = getComponents(pca_object, c(6)),
  rangeRetain = 0.1, absolute = TRUE,
  col = c('black', 'pink', 'red4'),
  drawConnectors = TRUE, labSize = 4, title = "Loadings for PC6") + coord_flip()
PC6_plotloadings
ggsave(paste(path, "results/03_11_PC6_plotloadings.png", sep="/"), plot = PC6_plotloadings, device = "png", width = 15, height = 15)
